...
import robLANG_ASOS.operations.data.LessData;
...
@Operationalize(node = Less.class, visitor = "robLANG_ASOS.interpreter.RobLANG_ASOSVisitor")
public class LessOp extends AdaptiveOperation<Less, robLANG_ASOSInterface>{
	
	@Override
	public Object execute(SelfAdaptiveVisitor vis, Less node, Object execCtx, robLANG_ASOSInterface config) {
		Object result = null;
		
		LessData data = new LessData();
		
		while(true){
			Object termination = null;
			LessData local_data = new LessData();
		
			boolean validPatternInput100 = true;
			...
			boolean validPatternInput101 = true && (data.getcomputed_left() instanceof Value);
			...
			boolean validPatternInput102 = true && (data.getcomputed_left() instanceof ValueInt) && (data.getcomputed_right() instanceof ValueInt);
			if(validPatternInput102)
			{
				if(config.before_less() != null){
					config.before_less().adapt(vis, node, data, config);
				}
				
				if(config.specialize_less() != null){
					config.specialize_less().adapt(vis, node, data, config);
				} else {
					Object n = (((ValueInt) data.getcomputed_left()).getValue() <= ((ValueInt) data.getcomputed_right()).getValue());
					ValueBool valueBool1 = SdFactory.eINSTANCE.createValueBool();
					valueBool1.setValue((java.lang.Boolean) n);
					ValueBool out = valueBool1;
					result = out;
				}
				
				if(config.after_less() != null){
					if(result == null){
						config.after_less().adapt(vis, node, data, config);
					} else {
						config.after_less().adapt(vis, ((AdaptableNode<robLANG_ASOSInterface>) result), null, config);
					}
				}
